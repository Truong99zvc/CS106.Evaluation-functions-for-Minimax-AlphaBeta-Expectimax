        ...
        def minValue(gameState, depth, agentIndex, alpha, beta):
            ...
            for action in gameState.getLegalActions(agentIndex):
                if agentIndex == gameState.getNumAgents() - 1:
                    value = min(value, maxValue(gameState.generateSuccessor(agentIndex, action), depth + 1, alpha, beta))
                else:
...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maxValue(gameState, depth):
            ...
            value = float("-Inf")
            for action in gameState.getLegalActions(0):
                value = max(value, expValue(gameState.generateSuccessor(0, action), depth, 1))
            return value
        ...
        def expValue(gameState, depth, agentIndex):
            ...
            for action in gameState.getLegalActions(agentIndex):
                if agentIndex == gameState.getNumAgents() - 1:
                    value += maxValue(gameState.generateSuccessor(agentIndex, action), depth + 1)
                else:
                    value += expValue(gameState.generateSuccessor(agentIndex, action), depth, agentIndex + 1)
...
def newbetterEvaluationFunction(currentGameState):
    """
    Hàm đánh giá trạng thái của Pacman dựa trên các đặc trưng sau:
    - Khoảng cách tới chấm thức ăn gần nhất
    - Khoảng cách tới con ma gần nhất
    - Khoảng cách tới capsule gần nhất
    - Số lượng chấm thức ăn còn lại
    - Số lượng capsule còn lại
    - Thời gian con ma sợ hãi
    - Khoảng cách tới góc tường gần nhất
    - Số lượng đường đi khả dụng
    """
    ...
    closest_ghost = min(manhattanDistance(pacman_position, ghost.getPosition()) for ghost in ghost_states) if ghost_states else 0
    closest_capsule = min(manhattanDistance(pacman_position, capsule) for capsule in capsules) if capsules else 0
    closest_food = min(manhattanDistance(pacman_position, food) for food in food_positions) if food_positions else 0
    remaining_food = len(food_positions)
    remaining_capsules = len(capsules)
    remaining_scared_time = max(scared_times) if scared_times else 0
    walls = currentGameState.getWalls().asList()
    closest_wall = min([util.manhattanDistance(pacman_position, wall) for wall in walls], default=0)
    available_paths = len(currentGameState.getLegalActions())

    # Trọng số cho các đặc trưng
    food_distance_weight = -100
    ghost_distance_weight = -0.2
    capsule_distance_weight = -5
    remaining_food_weight = -20
    remaining_capsules_weight = -30
    remaining_scared_time_weight = 2
    wall_distance_weight = 10
    available_paths_weight = 5

    evaluation = (
        food_distance_weight * closest_food +
        ghost_distance_weight / (closest_ghost + 1e-6) +
        capsule_distance_weight / (closest_capsule + 1e-6) +
        remaining_food_weight * remaining_food +
        remaining_capsules_weight * remaining_capsules +
        remaining_scared_time_weight * remaining_scared_time +
        wall_distance_weight / (closest_wall + 1e-6) +
        available_paths_weight * available_paths
    )
    ...
    if currentGameState.isLose():
        evaluation -= 500
    if [ghost for ghost in ghost_states if ghost.scaredTimer > 0]:
        evaluation += 200
    if food_positions:
        evaluation += 10 * len(food_positions)
